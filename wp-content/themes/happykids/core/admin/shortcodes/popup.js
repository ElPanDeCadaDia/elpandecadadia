	...
	$('#cws_insert_button').click( function() {
		...
		switch (type) {
			...
			case 'cws_button':
				...
				code_start = '[cws_button type=' + type + ' size=' + size + ' link=' + link + (fa ?  ' fa code=' + fa : "" ) + ' ';
				if (custom_color){
			...
			case 'shortcode_carousel':
				...
				code_start += "[shortcode_carousel" + ( title ? " title='" + title + "'" : "") + ( title ? " column='" + columns + "'" : "")+ "]" + selection + "[/shortcode_carousel]"
			break
		}
		if(window.tinyMCE) {
