...
$cws_shortcodes = array ('progress', 'embed', 'alert', 'cws_button', 'quote', 'fa', 'dropcap', 'mark', 'portfolio', 'cws_cta', 'news', 'tweets', 'shortcode_carousel', 'shortcode_blog');
...
function shortcode_cws_cta($attr, $content = null) {
	...
	$output .= "<div class='content_wrapper'><div class='text_part'>" . ( $title ? "<div class='title'>$title</div>" : "" ) . do_shortcode($content) . "</div><div class='button_part'><a class='cws_button large' href='$link'>$button_text</a></div></div>";
	$output .= '</div>';
...
function shortcode_button($atts, $content){
	...
	$out = "<a href='$link' class='cws_button " . $class . "'" . ( $custom_color ? " style='background:" . $button_color . ";" . " color:" . $text_color . "'" . " data-icon-color='" . $icon_color . "'" : "" ) . ">" . do_shortcode($content) . $fa_icon . "</a>";
	return $out;
}
...
function cws_shortcode_carousel ( $atts = array(), $content){
	...
	$column_number = empty($column) ? "" : 'data-carousel-column="'. $column .'"';

	$out = "<div class='shortcode_carousel' ".$column_number.">";
